'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = blurTouchActionReducer;

var _actionTypes = require('../../action-types');

var _actionTypes2 = _interopRequireDefault(_actionTypes);

var _updateField = require('../../utils/update-field');

var _updateField2 = _interopRequireDefault(_updateField);

var _getFieldForm = require('../../utils/get-field-form');

var _getFieldForm2 = _interopRequireDefault(_getFieldForm);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function blurTouchActionReducer(state, action, localPath) {
  if (action.type !== _actionTypes2.default.BLUR && action.type !== _actionTypes2.default.SET_TOUCHED) {
    return state;
  }

  var fieldForm = (0, _getFieldForm2.default)(state, localPath).$form;

  return (0, _updateField2.default)(state, localPath, function (fieldState) {
    return {
      focus: action.type === _actionTypes2.default.BLUR ? false : fieldState.focus,
      touched: true,
      retouched: !!(fieldForm.submitted || fieldForm.submitFailed)
    };
  });
}